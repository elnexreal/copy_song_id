...
struct CopySongID : Modify<CopySongID, LevelInfoLayer> {
	...
	*/
	...
	bool init(GJGameLevel *p0, bool p1) {
		...
		if (Loader::get()->isModLoaded("fleym.nongd")) {
			if (!m_fields->songWidget->m_isRobtopSong) {
				/*
					i can't access jukebox things bc i would need to make jukebox necessary to run my mod
					sorry for all the people who wanted to use my mod with jukebox, its going to be kinda broken
				*/

				auto moreBtn = m_fields->songWidget->m_moreBtn;
				auto menu = getChildOfType<CCMenu>(m_fields->songWidget, 0);

				auto spr = ButtonSprite::create("Copy");
				spr->setScale(0.55f);

				auto btn = CCMenuItemSpriteExtra::create(spr, this, menu_selector(CopySongID::onButtonClick));
				btn->setPosition({moreBtn->getPositionX() + 55.f, moreBtn->getPositionY()});
				btn->m_scaleMultiplier = 1.1f;
				btn->setID("copy-btn"_spr);
	...
	void levelDownloadFinished(GJGameLevel *level) {
		...
		if (!Loader::get()->getLoadedMod("fleym.nongd")) {
			if (m_fields->songWidget->m_isRobtopSong)
				return;
